...
class Player(QtWidgets.QMainWindow):
    ...
    def create_ui(self):
        ...
        # Create submenu to setup the server
        host_menu = QtWidgets.QMenu("Host", self)
        network_menu.addMenu(host_menu)
        ip_action = QtWidgets.QAction("Set ip", self)
        port_action = QtWidgets.QAction("Set port", self)
        host_action = QtWidgets.QAction("Host Server", self)
        host_menu.addAction(ip_action)
        host_menu.addAction(port_action)
        host_menu.addAction(host_action)

        # Create actions for network menu
        ip_action.triggered.connect(self.set_ip)
        port_action.triggered.connect(self.set_port)
        host_action.triggered.connect(self.host_server)
    ...
    def host_server(self):
        Server(self.ip_address, self.port_number, self.data_queue)
